...
public interface ProductDao extends JpaRepository<Product, Integer> {
	...
	@Query(value="select * from Products Where Status = 1 order by views DESC LIMIT 8", nativeQuery = true)
	List<Product> fillViewsDESC();
	
	@Query(value="select * from Products Where Status = 1 order by ManuDay DESC LIMIT 8", nativeQuery = true)
